...
import { isEqual, merge, set, startCase } from 'lodash';
...
function FuseSettings(props) {
  ...
  const formChanged = !isEqual(form, prevForm);
  const settingsChanged = !isEqual(settings, prevSettings);
  ...
  const ThemeSelect = ({ value, name, handleThemeChange }) => {
    return (
      <Select
        className="my-8 h-40 w-full overflow-hidden rounded-8"
        value={value}
        ...
        {Object.entries(themes)
          ...
          .map(([key, val]) => (
            ...
            >
              {startCase(key)}
              <div
                className="absolute bottom-0 left-0 right-0 block flex h-8 w-full"
                style={{
                  borderTop: `1px solid ${
